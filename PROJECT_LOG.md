...
- Agent List contact person (name, job title, phone, email) — schema + UI + import/export template. Requested twice now (this session and the session before).
- Full Contract PDF redesign ("EXHIBITION SPACE APPLICATION FORM") — detailed spec given twice (once verbally, once re-stated in full this session with two reference files attached: a DRAFT docx and a real system-generated PDF the user calls "ugly"). Key requirements to preserve for whoever picks this up: logo follows Company Profile; Section 1 ties to Event (needs a new `venue` field on Event Edition, full-width in the template — already exists as `events.venue` from an earlier migration, just needs wiring into the PDF); Section 2 ties to Exhibitor detail; Section 3 (billing) only shows active Price List items with a nonzero amount, no item codes shown to the exhibitor, booths (primary + upgrade) on top with other Price List items below; Preferred Hall left blank if it can't fit all booths taken; Remarks linked to the remarks field; add an optional Stamp Duty billing line; Section 5 needs an admin-editable payment-terms/declaration wording template (mirroring the existing "EXHIBITOR'S DECLARATION AND SIGNATURE" pattern, applies to MIFB27 so far); banking details link to Company Profile; whole document constrained to 2 pages.

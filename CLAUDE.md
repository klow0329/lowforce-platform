...
## 12. Password policy: min 8 chars, upper/lower/digit/special — one shared validator (2026-08-04)
Every place a password is SET (tenant self-service change, tenant Admin's manual reset / Create User, platform admin self-service change) must go through `backend/src/utils/passwordPolicy.js`'s `passwordPolicyError()` — never a local `.length < 8` check. Every system-generated temp password (platform console's create-first-admin / reset-user-password, Admin > Users bulk import) must use that same file's `generateStrongPassword()`, which deterministically hits all four required character classes rather than hoping randomness does. This exists so a freshly system-generated login never fails the same policy that gates it, and so the rule only needs to change in one place.
- Existing passwords are **not** retroactively invalidated — the policy only applies when a password is being set, not checked at login. A pre-existing password that doesn't meet it keeps working until it's next changed.

## 13. SSO — approved, not yet built (2026-08-04)
User approved dual login (password OR SSO) for **Google and Microsoft** both, with an account-chooser dropdown so a user with multiple emails on their device can pick which one to sign in with. Password policy (rule #12) was tightened as part of this same decision (originally floated as "6 chars," kept at the existing 8-char floor since it's strictly stronger and the real ask was complexity).
- **Not started** — needs the user to first create an OAuth Client (Google Cloud Console) and an App Registration (Microsoft Entra ID) in their own accounts; Claude cannot create third-party app credentials on the user's behalf. Once Client ID/Secret exist for both, the build is: OAuth callback routes, linking an authenticated Google/Microsoft email to an existing `users` row by email match (this is a B2B multi-tenant system — SSO must not auto-create accounts across arbitrary companies, same email-then-company-picker pattern as password login), and a "Sign in with Google / Microsoft" option alongside the existing password form.
- Per rule #4, this is a free OAuth flow (Google/Microsoft don't charge per-user for basic sign-in), not a paid identity vendor (Auth0/Clerk/Firebase Auth) — does not conflict with the no-third-party-auth-vendor rule.
